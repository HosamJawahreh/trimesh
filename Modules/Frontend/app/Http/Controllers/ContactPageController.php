...
class ContactPageController extends Controller implements HasMiddleware
{
    ...
    {
        // Show only the default contact card on the public contact page.
        // Fallback: if no contact is marked default, return the first contact available.
        $contacts = Contact::where('default', true)->get();
        if ($contacts->isEmpty()) {
            $contacts = Contact::orderBy('id')->take(1)->get();
        }
