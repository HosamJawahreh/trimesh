...
<form action="{{ route('admin.faqs.store') }}" method="POST">
    ...
    <x-card :heading="__('admin.add_new_faq')" :route="route('admin.faqs.index')" btnType="back">
        ...
        <div class="mb-4">
            <x-input-label for="code" :value="__('attribute.language')" />
            <select id="code" name="code" class="form-control" required>
                <option value="">{{ __('admin.select_language') }}</option>
                <option value="en" {{ old('code', app()->getLocale()) == 'en' ? 'selected' : '' }}>English</option>
                <option value="ar" {{ old('code', app()->getLocale()) == 'ar' ? 'selected' : '' }}>Arabic</option>
